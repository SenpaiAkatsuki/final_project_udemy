...
async def confirm_feedback_answer(message: types.Message, state: FSMContext):
    ...
    reports = await db.select_all_reports()
    if re.compile("^.{1,1000}$").match(message.text):
        ...
        await message.bot.send_message(user['telegram_id'],
                                       ...
                                       f"Ответ от администратора <b><code>{message.from_user.full_name}</code></b>:")
        await message.bot.send_message(user['telegram_id'],
                                       text=f"{message.text}")

        await message.answer("Ответ отправлен пользователю☑️",
...
def register_feedback_answer_handler(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(catch_feedback_answer,
                                       ...
                                       state=Feedback.answerFeedback)
    dp.register_message_handler(confirm_feedback_answer, state=Feedback.getFeedbackAnswer)
